...
#ifndef __CS32L010_HAL_RCC_H
#define __CS32L010_HAL_RCC_H
...
typedef struct
{
  uint32_t ClockType;             // 需要配置的时钟  @ref RCC_System_Clock_Type
  uint32_t SYSCLKSource;          // 作为系统时钟(SYSCLKS)的时钟源  @ref RCC_System_Clock_Source
  uint32_t AHBCLKDivider;         // AHB时钟(HCLK)分频器。 这个时钟来自于系统时钟(SYSCLK)。 取值范围为0 ~ 255，默认为RCC_HCLK_DIV1
  uint32_t APBCLKDivider;         // APB1时钟(PCLK1)分频器。 这个时钟来自于AHB时钟(HCLK)。取值范围为0 ~ 255，默认为RCC_PCLK_DIV1
} RCC_ClkInitTypeDef;
...
typedef struct
{
  uint32_t OscillatorType;       //要配置的振荡器.该参数可以是 @ref RCC_Oscillator_Type
  uint32_t HIRCState;              // The new state of the HIRC. @ref RCC_HIRC_Config
  uint32_t HIRCCalibrationValue;   // HIRC校准微调值. @ref RCC_HIRC_Calibration_Config
  uint32_t HXTState;              // The new state of the HXT. @ref RCC_HXT_Config
  uint32_t LIRCState;              //  The new state of the LIRC. @ref RCC_LIRC_Config
  uint32_t LIRCStartupValue;       //  LIRC启动时间. @ref RCC_LIRC_Startup_Value
  uint32_t LIRCCalibrationValue;   // LIRC校准微调值 . @ref RCC_LIRC_Calibration_Config
  uint32_t LXTState;               //  The new state of the LXT. @ref RCC_LXT_Config
