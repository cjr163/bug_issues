...
#ifndef __CS32L010_HAL_GPIO_H
#define __CS32L010_HAL_GPIO_H
...
typedef struct
{
  uint32_t Pin;       	    //需要配置的GPIO引脚。 @ref GPIO_pins_define
  uint32_t Mode;    	    //指定所选引脚的工作模式  @ref GPIO_mode_define
  uint32_t Pull;     	    //指定选定引脚的上拉或下拉激活  @ref GPIO_pull_define
  uint32_t OpenDrain;       // 指定所选引脚的 推挽 或 开漏 输出  @ref GPIO_opendrain_define
  GPIO_Debounce_InitTypeDef Debounce;   //消抖 防反跳  @ref GPIO_debounce_define
  uint32_t SlewRate;        //指定所选引脚的转换速率。 @ref GPIO_slewrate_define
  uint32_t DrvStrength; 	// 指定所选引脚的驱动强度。  @ref GPIO_driver_strength_define
	GPIO_Exti_InitTypeDef Exti;		//指定所选引脚的exti类型  @ref GPIO_exti_define
  uint32_t Alternate;  		// 要连接到所选引脚的外围设备。  @ref GPIO_alternate_function_selection
} GPIO_InitTypeDef;
