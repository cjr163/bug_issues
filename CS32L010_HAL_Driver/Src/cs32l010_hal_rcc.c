...
/**
  * @brief  根据 RCC_OscInitTypeDef 中指定的参数初始化RCC振荡器.
  * @param  RCC_OscInitStruct 结构体
  * @note  	重要! 如果使用TSSOP-20和QFN-20包装产品，应定义PACKAGE宏以选择正确的校准值。
  * 否则，如果使用KGD(晶圆)产品(OTP?)，包装宏不应该被定义。 详细内容请参见《用户手册》6.4.10章
  * @retval HAL status
  */
...
/**
  * @brief  根据 RCC_ClkInitStruct 中指定的参数初始化CPU、AHB和APB总线时钟。
  * @param  RCC_ClkInitStruct 结构体
  * @note   SystemCoreClock CMSIS变量用于存储系统时钟频率，并由该函数中调用的 HAL_RCC_GetHCLKFreq() 函数更新

  * @note   只有当目标时钟源准备就绪时，才会发生时钟源切换。
            如果选择“未准备好”时钟源，则时钟源准备好后，交换机才会发生切换。
            你可以使用 HAL_RCC_GetClockConfig() 函数来知道当前哪个时钟被用作系统时钟源。
  * @retval HAL status
  */
...
/**
  ...
  * @note     HIRC_VALUE_xxM is a constant defined in cs32l010_hal_conf.h file (default value
  *           4 MHz) but the real value may vary depending on the variations in voltage and temperature.
  * @note     HXT_VALUE is a constant defined in cs32l010_hal_conf.h file (default value
  *           8 MHz), user has to ensure that HSE_VALUE is same as the real
  *           frequency of the crystal used. Otherwise, this function may
  *           have wrong result.
  * @note     LIRC_VALUE is a constant defined in cs32l010_hal_conf.h file (default value
  *           38.4 KHz) but the real value may vary depending on the variations
  *           in voltage and temperature.
  * @note     LXT_VALUE is a constant defined in cs32l010_hal_conf.h file (default value
